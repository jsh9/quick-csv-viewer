...
<head>
  ...
  <style nonce="${nonce}">
    ...
    * {
    ...
    .table-row {
      display: grid;
      grid-template-columns: 58px repeat(var(--column-count), minmax(140px, 1fr));
    }
...
<body>
  ...
  <script nonce="${nonce}">
    ...
    function createTableShell(payload, columnCount) {
      ...
      scroll.style.setProperty('--column-count', String(Math.max(1, columnCount)));
      scroll.style.setProperty('--table-min-width', Math.max(158, 58 + columnCount * 140) + 'px');
      ...
      if (payload.firstRowIsHeader) {
        ...
        for (const label of getHeaders(payload, columnCount)) {
    ...
    function getBodyViewport() {
      const header = virtualScroll ? virtualScroll.querySelector('.table-header') : null;
